    ...
    <div className="space-y-1">
      {Array.from({ length: 5 }).map((_, i) => (
        <div key={i} className="flex items-start gap-3 rounded-lg px-3 py-2.5">
          <div className="bg-bg-tertiary mt-0.5 h-5 w-5 animate-pulse rounded-full" />
...
export function TaskList({
  ...
  if (isLoading) {
    return (
      <div className={cn('space-y-1', className)}>
        {title && <h2 className="text-text-secondary mb-2 px-3 text-sm font-semibold">{title}</h2>}
        <TaskListSkeleton />
      </div>
    );
  }
  ...
  return (
    <div className={cn('space-y-0.5', className)}>
      ...
      {tasks.length === 0 ? (
        <div className="text-text-tertiary flex flex-col items-center justify-center py-12">
