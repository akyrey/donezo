...
export function CommandPalette({ open, onOpenChange }: CommandPaletteProps) {
    ...
    function renderIcon(type: string) {
        switch (type) {
            ...
            case 'project':
                return <FolderOpen className="h-4 w-4 text-text-tertiary" />;
            case 'section':
                return <LayoutList className="h-4 w-4 text-text-tertiary" />;
            case 'tag':
                return <Tag className="h-4 w-4 text-text-tertiary" />;
            default:
                return null;
        }
    }
        ...
        <DialogPrimitive.Root open={open} onOpenChange={onOpenChange}>
            <DialogPrimitive.Portal>
                ...
                <DialogPrimitive.Content
                    className={cn(
                        'fixed left-1/2 top-[20%] z-50 w-full max-w-lg -translate-x-1/2 overflow-hidden rounded-xl border border-border bg-bg shadow-2xl',
                ...
                >
                    ...
                    <div className="flex items-center gap-3 border-b border-border px-4">
                        ...
                        <input
                            ...
                            className="h-12 flex-1 bg-transparent text-sm text-text outline-none placeholder:text-text-tertiary"
                            autoFocus
                        />
                        {isLoading && (
                            <Loader2 className="h-4 w-4 shrink-0 animate-spin text-text-tertiary" />
                        )}
                        <kbd className="hidden shrink-0 rounded border border-border bg-bg-secondary px-1.5 py-0.5 text-[10px] font-medium text-text-tertiary sm:inline-block">
                            ESC
                        </kbd>
                    </div>
                    ...
                    >
                        ...
                        {!query.trim() && (
                            <div>
                                <p className="mb-1 px-2 text-xs font-semibold uppercase tracking-wider text-text-tertiary">
                                    Navigate
                                </p>
                                {flatItems.map((item, index) => (
                                    ...
                                    >
                                        {renderNavIcon(
                                            NAVIGATION_COMMANDS[index]?.href ?? '',
                                        )}
                                        {item.label}
                                    </button>
                                ))}
                    ...
                    <div className="flex items-center justify-between border-t border-border px-4 py-2">
                        <div className="flex items-center gap-3 text-xs text-text-tertiary">
                            <span className="flex items-center gap-1">
                                <kbd className="rounded border border-border bg-bg-secondary px-1 py-0.5 text-[10px] font-medium">
                                    &uarr;
                                </kbd>
                                <kbd className="rounded border border-border bg-bg-secondary px-1 py-0.5 text-[10px] font-medium">
                                    &darr;
