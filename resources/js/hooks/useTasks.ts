...
async function fetchTasks(filters: TaskFilters = {}): Promise<TasksResponse> {
  ...
  const { data } = await axios.get<TasksResponse>(`/api/v1/tasks?${params.toString()}`);
  return data;
}
...
export function useTaskMutation() {
  ...
  return useMutation({
    mutationFn: async (data: TaskCreateData) => {
      const response = await axios.post<{ data: Task }>('/api/v1/tasks', data);
...
export function useUpdateTaskMutation() {
  ...
  return useMutation({
    mutationFn: async ({ id, ...data }: TaskUpdateData & { id: number }) => {
      const response = await axios.put<{ data: Task }>(`/api/v1/tasks/${id}`, data);
      return response.data.data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: TASKS_KEY });
  ...
  return useMutation({
    mutationFn: async (id: number) => {
      const response = await axios.post<{
        data: { id: number; title: string; is_completed: boolean; position: number };
