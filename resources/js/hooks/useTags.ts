...
export function useTagMutation() {
  ...
  return useMutation({
    mutationFn: async (data: TagCreateData) => {
      const response = await axios.post<{ data: Tag }>('/api/v1/tags', data);
...
export function useUpdateTagMutation() {
  ...
  return useMutation({
    mutationFn: async ({ id, ...data }: TagUpdateData & { id: number }) => {
      const response = await axios.put<{ data: Tag }>(`/api/v1/tags/${id}`, data);
      return response.data.data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: TAGS_KEY });
