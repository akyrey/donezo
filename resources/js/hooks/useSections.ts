...
export function useSectionsQuery() {
  return useQuery<SectionsResponse>({
    ...
    queryFn: async () => {
      const { data } = await axios.get<SectionsResponse>('/api/v1/sections');
      return data;
    },
  });
}
...
export function useSectionMutation() {
  ...
  return useMutation({
    mutationFn: async (data: SectionCreateData) => {
      const response = await axios.post<{ data: Section }>('/api/v1/sections', data);
...
export function useUpdateSectionMutation() {
  ...
  return useMutation({
    mutationFn: async ({ id, ...data }: SectionUpdateData & { id: number }) => {
      const response = await axios.put<{ data: Section }>(`/api/v1/sections/${id}`, data);
      return response.data.data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: SECTIONS_KEY });
