        ...
        <div className="mb-8">
          ...
          {tasks.meta.total > 0 && (
            <p className="text-text-secondary mt-1 text-sm">
              {tasks.meta.total} completed {tasks.meta.total === 1 ? 'task' : 'tasks'}
            </p>
          )}
        </div>
        ...
        {tasks.data.length > 0 ? (
          <>
            ...
            {tasks.meta.last_page > 1 && (
              <div className="border-border mt-8 flex items-center justify-between border-t pt-4">
                <Button variant="ghost" size="sm" disabled={!prevLink} asChild={!!prevLink}>
                  {prevLink ? (
                    <Link href={prevLink}>
                ...
                <Button variant="ghost" size="sm" disabled={!nextLink} asChild={!!nextLink}>
                  {nextLink ? (
                    <Link href={nextLink}>
                      Next
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            <div className="bg-bg-secondary mb-4 flex h-16 w-16 items-center justify-center rounded-full">
              <BookOpen className="text-text-tertiary h-8 w-8" />
            </div>
            <h2 className="text-text text-lg font-medium">No completed tasks yet</h2>
            <p className="text-text-secondary mt-1 text-sm">
      ...
      >
        <DialogContent className="max-w-sm">
          ...
          <DialogFooter className="mt-2 gap-2">
            <Button variant="ghost" onClick={() => setTaskToUncomplete(null)}>
              Cancel
            </Button>
            <Button onClick={handleConfirmUncomplete} disabled={completeTask.isPending}>
