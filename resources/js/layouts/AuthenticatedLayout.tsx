  ...
  return (
    <Link
      ...
      className={cn(
        ...
        isDragging && isOver && 'ring-primary bg-primary/10 text-primary ring-2 ring-inset',
      )}
    >
      <Icon
        className={cn(
          'h-4.5 w-4.5 shrink-0',
          active || (isDragging && isOver) ? 'text-primary' : 'text-text-tertiary',
        )}
      />
      ...
      {showCount && (
        <span
          className={cn(
            'ml-auto min-w-[1.25rem] rounded-full px-1.5 py-0.5 text-center text-xs leading-none font-medium tabular-nums',
    ...
    >
      ...
      >
        <FolderOpen
          className={cn(
            'h-4 w-4 shrink-0',
            isDragging && isOver ? 'text-primary' : 'text-text-tertiary',
          )}
        />
      ...
      <div className="relative h-5 w-5 shrink-0">
        {showCount && (
          <span
            className={cn(
              'absolute inset-0 hidden items-center justify-center rounded-full text-xs leading-none font-medium tabular-nums lg:flex lg:group-hover/item:invisible',
              active || (isDragging && isOver)
                ? 'bg-primary/15 text-primary'
                : 'bg-bg-tertiary text-text-tertiary',
            )}
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button
              className={cn(
                'text-text-tertiary hover:text-text absolute inset-0 flex items-center justify-center rounded transition-colors lg:invisible lg:group-hover/item:visible',
...
function DroppableSectionItem({
  ...
  return (
    <div
      ref={setNodeRef}
      className={cn(
        'group/item text-text-secondary flex items-center gap-3 rounded-lg px-3 py-2 text-sm transition-colors',
        isDragging && 'hover:bg-sidebar-hover cursor-copy',
        isDragging && isOver && 'ring-primary bg-primary/10 text-primary ring-2 ring-inset',
...
function DraggableFab({ onTap, isDragging }: { onTap: () => void; isDragging: boolean }) {
  const {
    attributes,
    listeners,
    setNodeRef,
      ...
      { name: name.trim() },
      {
        ...
        onError: (err: unknown) => {
          const axiosError = err as {
            response?: { data?: { errors?: { name?: string[] }; message?: string } };
          };
          const message =
    ...
    >
      <Dialog.Portal>
        ...
        <Dialog.Content className="border-border bg-bg text-text data-[state=open]:animate-in data-[state=open]:fade-in-0 data-[state=open]:zoom-in-95 data-[state=open]:slide-in-from-left-1/2 data-[state=open]:slide-in-from-top-[48%] fixed top-1/2 left-1/2 w-full max-w-md -translate-x-1/2 -translate-y-1/2 rounded-2xl border p-6 shadow-lg focus:outline-none">
          <Dialog.Title className="text-text text-lg font-semibold">New Section</Dialog.Title>
  ...
  return (
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        <Dialog.Overlay className="data-[state=open]:animate-in data-[state=open]:fade-in-0 data-[state=closed]:animate-out data-[state=closed]:fade-out-0 fixed inset-0 bg-black/40" />
        <Dialog.Content className="border-border bg-bg text-text data-[state=open]:animate-in data-[state=open]:fade-in-0 data-[state=open]:zoom-in-95 data-[state=open]:slide-in-from-left-1/2 data-[state=open]:slide-in-from-top-[48%] fixed top-1/2 left-1/2 w-full max-w-md -translate-x-1/2 -translate-y-1/2 rounded-2xl border p-6 shadow-lg focus:outline-none">
          <Dialog.Title className="text-text text-lg font-semibold">Rename {label}</Dialog.Title>
          <Dialog.Description className="text-text-secondary mt-1 text-sm">
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="flex h-14 items-center px-5">
        <Link href="/" className="text-text text-lg font-bold tracking-tight" onClick={onNavigate}>
          Donezo
        </Link>
      </div>
      ...
      <ScrollArea className="flex-1 px-3">
        ...
        <div className="py-2">
          ...
          <div className="space-y-0.5">
            ...
            {projects.length === 0 && (
              <p className="text-text-tertiary px-3 py-2 text-xs">No projects yet</p>
            )}
          </div>
        </div>
        ...
        <div className="py-2">
          ...
          <div className="space-y-0.5">
            ...
            {sections.length === 0 && (
              <p className="text-text-tertiary px-3 py-2 text-xs">No sections yet</p>
            )}
          </div>
        </div>
        ...
        <div className="py-2">
          ...
          <div className="space-y-0.5">
            ...
            {groups.length === 0 && (
              <p className="text-text-tertiary px-3 py-2 text-xs">No groups yet</p>
            )}
          </div>
        </div>
      </ScrollArea>
      ...
      <div className="border-border border-t p-3">
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button className="hover:bg-sidebar-hover flex w-full items-center gap-3 rounded-lg px-3 py-2 text-sm transition-colors">
              <div className="bg-primary flex h-7 w-7 items-center justify-center rounded-full text-xs font-medium text-white">
                {user.name
                  .split(' ')
                  .map((n: string) => n[0])
                  .join('')
...
export default function AuthenticatedLayout({
  ...
  return (
    <DndContext sensors={sensors} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
      <div className="bg-bg-secondary text-text flex h-screen">
        {/* Mobile overlay */}
        {sidebarOpen && (
          <div
        ...
        <aside
          className={cn(
            'border-border bg-sidebar fixed inset-y-0 left-0 z-50 w-64 transform border-r transition-transform duration-250 ease-in-out lg:static lg:translate-x-0',
            sidebarOpen ? 'translate-x-0' : '-translate-x-full',
          )}
        >
          {/* Mobile close button */}
          <div className="absolute top-3 right-2 lg:hidden">
            <Button
              variant="ghost"
              size="icon"
        ...
        <main className="flex flex-1 flex-col overflow-hidden">
          {/* Top bar */}
          <header className="border-border-light bg-bg flex h-14 items-center gap-3 border-b px-4 lg:px-6">
            <Button
              variant="ghost"
              size="icon"
              className="lg:hidden"
            ...
            {title && <h1 className="text-text text-lg font-semibold">{title}</h1>}
            ...
            >
              ...
              <kbd className="border-border bg-bg-secondary text-text-tertiary hidden rounded border px-1.5 py-0.5 text-[10px] font-medium sm:inline-block">
        ...
        <DragOverlay dropAnimation={null}>
          {isDragging && (
            <div className="bg-primary flex h-14 w-14 scale-110 items-center justify-center rounded-full text-white opacity-90 shadow-2xl">
              <Plus className="h-6 w-6" />
            </div>
          )}
        </DragOverlay>
        ...
        {/* Add Section Dialog */}
        <AddSectionDialog open={addSectionOpen} onOpenChange={setAddSectionOpen} />
        ...
        {/* Global toast notifications (driven by showToast() / QueryClient onError) */}
        <ToastRegion />
      </div>
    </DndContext>
  );
